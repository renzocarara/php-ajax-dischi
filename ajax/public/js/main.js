...
var Handlebars = __webpack_require__(/*! handlebars */ "./node_modules/handlebars/dist/cjs/handlebars.js"); // const template = Handlebars.compile("Name: {{name}}");
  ...
  $('#card-genre').change(function () {
    ...
    switch (genreSelected) {
      ...
      default:
        ...
        $('.select-container i').fadeIn();
      // visualizzo tute le icone
    } // chiamata AJAX per recuperare dati dal DB virtuale dischi.php
...
function handleResponseData(cardsData) {
  ...
  var cardsDataJsObj = JSON.parse(cardsData); // recupero il codice html dal template HANDLEBARS

  var cardTemplate = $('#music-card-template').html(); // compilo il template HANDLEBARS, lui mi restituisce un funzione

  var cardFunction = Handlebars.compile(cardTemplate); // svuoto la pagina dalle cards prima di riempirla coi nuovi dati

  $('.cards-container').empty(); // ciclo su tutto l'array composto dai dati ricevuti dal server
